...
class Zomato:
    ...
    def restaurant_search(self, query="", latitude="", longitude="", cuisines="", limit=5 , \
        ...
        This function has been modified to do a sort and order by the average rating of the restaurant
        """
        cuisines = "%2C".join(cuisines.split(","))
        if str(limit).isalpha() == True:
            raise ValueError('LimitNotInteger')
        headers = {'Accept': 'application/json', 'user-key': self.user_key}
        r = (requests.get(base_url + "search?q=" + str(query) + "&count=" + str(limit) + "&lat=" + str(latitude) + "&lon=" + str(longitude) + \
             "&cuisines=" + str(cuisines) \
             + "&sort=" + str(sort) + "&order=" + str(order), \
    ...
    def restaurant_search_by_keyword(self, query="", cuisines="", limit=5 ,sort="rating", order="desc", average_cost_for_two="300"):
        """
        Takes either query, latitude and longitude or cuisine as input.
        Returns a list of Restaurant IDs.
        """
        ...
        r = (requests.get(base_url + "search?q=" + str(query) + "&count=" + str(limit) + "&cuisines=" + str(cuisines) \
             + "&average_cost_for_two=" + str(average_cost_for_two) + "&sort=" + str(sort) + "&order=" + str(order),headers=headers).content).decode("utf-8")
        return r
